...
############ prepare dataset #######
junkData = data.frame(groupData, rep(LLs, 3), rep(solutions[,1], 3), rep(solutions[,2], 3), rep(solutions[,3], 3))
...
for(cIdx in 1 : 2){
  ...
  ggplot(plotData, aes(auc, phi)) + geom_point() + saveTheme + ggtitle(cond) + geom_smooth(method = 'lm') +
    geom_text(aes(label = phiLabel),x = 15, y = 0.1, size = 6, color = 'blue') + facet_grid(.~blockNum)
  ...
  ggplot(plotData, aes(auc, gamma)) + geom_point() + saveTheme + ggtitle(cond) + geom_smooth(method = 'lm') +
    geom_text(aes(label = gammaLabel),x = 20, y = 0.3, size = 6, color = 'blue') + facet_grid(.~blockNum)
